...
@interface BYAudioHudView() <BYAudioRecorderDelegate>
...
@property (nonatomic, strong) BYTimerLabel *timeLabel;
...
- (void)configRecordingHUDImageWithPeakPower:(CGFloat)peakPower {
    ...
    NSString *path = [NSString stringWithFormat:@"Frameworks/BYAudioHud.framework/%@", imageName];
    UIImage *image = [UIImage imageNamed:path];
    self.recordingHUDImageView.image = image;
}
...
- (void)setup {
    ...
    if (!_recordingHUDImageView) {
        ...
        NSString *path = [NSString stringWithFormat:@"Frameworks/BYAudioHud.framework/%@", @"RecordingSignal001"];
        UIImage *image = [UIImage imageNamed:path];
        recordHUDImageView.image = image;

 ...
 */
static const Float64 kDefaultSampleRate = 8000;
//#define kDefaultSampleRate 8000

#define kBYAudioRecorderErrorDomain @"BYAudioRecorderErrorDomain"

#define IfAudioQueueErrorPostAndReturn(operation,error) \
do{\
if(operation!=noErr) { \
[self postAErrorWithErrorCode:BYRecorderErrorCodeAboutQueue andDescription:error]; \
return; \
}   \
}while(0)

@interface BYAudioRecorder()
{
    //音频输入缓冲区
...
@implementation BYAudioRecorder

- (instancetype)init {
    self = [super init];
    if (self) {
        //建立写入文件线程队列,串行，和一个信号量标识
        self.writeFileQueue = dispatch_queue_create("com.ilei.BYAudioRecorder.writeFileQueue", NULL);
...
void inputBufferHandler(void *inUserData, AudioQueueRef inAQ, AudioQueueBufferRef inBuffer, const AudioTimeStamp *inStartTime,UInt32 inNumPackets, const AudioStreamPacketDescription *inPacketDesc) {
    BYAudioRecorder *recorder = (__bridge BYAudioRecorder*)inUserData;
                ...
                if(recorder.fileWriterDelegate&&![recorder.fileWriterDelegate writeIntoFileWithData:pcmData withRecorder:recorder inAQ:inAQ inStartTime:inStartTime inNumPackets:inNumPackets inPacketDesc:inPacketDesc]){
                    ...
                    if (dispatch_semaphore_wait(recorder.semError,DISPATCH_TIME_NOW)==0){
                        ...
                        dispatch_async(dispatch_get_main_queue(),^{
                            [recorder postAErrorWithErrorCode:BYRecorderErrorCodeAboutFile andDescription:@"写入文件失败"];
                        });
                    }
                }
            });
    ...
    if (recorder.isRecording) {
        if(AudioQueueEnqueueBuffer(inAQ, inBuffer, 0, NULL)!=noErr){
            ...
            //回到主线程
            dispatch_async(dispatch_get_main_queue(),^{
                [recorder postAErrorWithErrorCode:BYRecorderErrorCodeAboutQueue andDescription:@"重准备音频输入缓存区失败"];
            });
        }
    }
}
...
- (void)startRecording {
    ...
    if (!ret) {
        [self postAErrorWithErrorCode:BYRecorderErrorCodeAboutSession andDescription:@"为AVAudioSession设置Category失败"];
        return;
    }
    ...
    if (!ret) {
        [self postAErrorWithErrorCode:BYRecorderErrorCodeAboutSession andDescription:@"Active AVAudioSession失败"];
        return;
    }
    ...
    if (!self.fileWriterDelegate||![self.fileWriterDelegate respondsToSelector:@selector(createFileWithRecorder:)]||![self.fileWriterDelegate respondsToSelector:@selector(writeIntoFileWithData:withRecorder:inAQ:inStartTime:inNumPackets:inPacketDesc:)]||![self.fileWriterDelegate respondsToSelector:@selector(completeWriteWithRecorder:withIsError:)]){
        [self postAErrorWithErrorCode:BYRecorderErrorCodeAboutOther andDescription:@"fileWriterDelegate的代理未设置或其代理方法不完整"];
        return;
    }
    ...
    dispatch_sync(self.writeFileQueue, ^{
        if(self.fileWriterDelegate&&![self.fileWriterDelegate createFileWithRecorder:self]){
            dispatch_async(dispatch_get_main_queue(),^{
                [self postAErrorWithErrorCode:BYRecorderErrorCodeAboutFile andDescription:@"为音频输入建立文件失败"];
            });
            isContinue = NO;
        }
    });
    ...
    if (self.isRecording) {
        ...
        dispatch_sync(self.writeFileQueue, ^{
            if (self.fileWriterDelegate&&![self.fileWriterDelegate completeWriteWithRecorder:self withIsError:NO]) {
                dispatch_async(dispatch_get_main_queue(),^{
                    [self postAErrorWithErrorCode:BYRecorderErrorCodeAboutFile andDescription:@"为音频输入关闭文件失败"];
                });
                isContinue = NO;
            }
        });
...
- (void)postAErrorWithErrorCode:(BYRecorderErrorCode)code andDescription:(NSString*)description {
    //关闭可能还未关闭的东西,无需考虑结果
    self.isRecording = NO;
    ...
    NSError *error = [NSError errorWithDomain:kBYAudioRecorderErrorDomain code:code userInfo:@{NSLocalizedDescriptionKey:description}];

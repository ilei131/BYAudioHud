...
#define kBYAudioMeterObserverErrorDomain @"BYAudioMeterObserverErrorDomain"

#define IfAudioQueueErrorPostAndReturn(operation,error) \
if(operation!=noErr) { \
[self postAErrorWithErrorCode:BYAudioMeterObserverErrorCodeAboutQueue andDescription:error]; \
return; \
}   \
...
@interface BYAudioMeterObserver()
...
@implementation BYAudioMeterObserver
...
- (void)postAErrorWithErrorCode:(BYAudioMeterObserverErrorCode)code andDescription:(NSString*)description {
    self.audioQueue = nil;
    //DLOG(@"监控音频队列光谱发生错误");
    NSError *error = [NSError errorWithDomain:kBYAudioMeterObserverErrorDomain code:code userInfo:@{NSLocalizedDescriptionKey:description}];
    if( self.errorBlock){
